...
<html>
    ...
    <?php
        echo "<h1 align=left>Choose an artist</h1>";
                    require_once "../config.php";
                    require_once "admin_check.php";

                    $elec="SELECT * FROM `artists`";
                    $qr= $con ->query($elec) or die($con->error);

                    if(!$qr || mysqli_num_rows($qr) > 0)
                    {
                        echo"<table><thead><td>Artist</td><td>Link To Sales Page</td></thead><tr>";
                        while($row = $qr->fetch_assoc()) {
                            echo"<div id='user'>";
                            echo "<td>".$row['Artist_Name']."</td>";
                            echo"<td><a href=art_all_view.php?ID=" .$row['Artist_ID'].">View</a></td>"; 
                            echo"</div> </tr>";
